...
from test.utils import random_genres_ids, random_director_id, random_film_id, last_genre_id, last_film_id
...
def test_success(data: dict, director_factory, film_factory,
    ...
    response = test_client.patch(f'/api/v1/films/{film_id}', json=data)
...
def test_user_unauthorized(director_factory, film_factory, test_client: FlaskClient):
    director_factory(1)
    film_factory(1)
    response = test_client.patch(f'/api/v1/films/{random_film_id()}')
...
def test_validate_error(data: dict, expected_loc: t.Tuple[str],
                        ...
                        director_factory, film_factory,
                        loging_user: RegistrationUserSchema, test_client: FlaskClient):
    director_factory(1)
    film_factory(1)
    response = test_client.patch(f'/api/v1/films/{random_film_id()}', json=data)
...
def test_user_name_exist(data: dict, director_factory, film_factory,
                         loging_user: RegistrationUserSchema, test_client: FlaskClient):
    director_factory(1)
    film_factory(1)
    data['genres_id'] = random_genres_ids()
    data['director_id'] = random_director_id()
    response = test_client.patch(f'/api/v1/films/{last_film_id() + 1}', json=data)
    assert response.status_code == 404

    resp_data = response.get_json()
    assert resp_data['msg'] == 'film don\'t exists'
...
def test_user_not_owner(data: dict, director_factory, film_factory,
    ...
    resp_data = response.get_json()
    assert resp_data['msg'] == 'permission denied'
